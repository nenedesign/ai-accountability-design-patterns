...
At acquisition, a cryptographic hash (SHA-256, MD5) is computed for each artifact. At any subsequent point, the hash can be recomputed and compared to the original. If they match, the artifact is unchanged. Standard practice in digital forensics (NIST, SWGDE, ACPO) and implemented in all major forensic acquisition tools.

### Tamper Indicator [Established]
A visible alert in the interface when an artifact's hash does not match its original value, or when a record has been modified outside expected parameters. The indicator surfaces the integrity failure at the point of use, not only in a background log. Implemented in forensic evidence management systems and tamper-evident logging platforms.
...
At the moment an artifact is acquired or a record is created, it is cryptographically signed. The signature verifies both the content (unchanged) and the creator (authenticated). Implemented in forensic acquisition tools and document signing platforms.
